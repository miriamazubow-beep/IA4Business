# Dificuldade de mapear e fechar parcerias de marca

## Quem sofre
Camila Duarte, criadora de conteúdo de lifestyle e maternidade no Instagram e TikTok (@camiladuarte, 95 mil seguidores no Instagram e 60 mil no TikTok), atua sozinha, sem agência, e concilia a produção de conteúdo com o cuidado de dois filhos pequenos em Belo Horizonte.

## Como é hoje
Hoje a influencer que busca parcerias de marca geralmente:
- Recebe propostas espalhadas por e-mail, DM do Instagram/TikTok e WhatsApp, sem um lugar único para acompanhar.
- Mantém controle manual (ou nenhum controle) do status de cada parceria: quem já respondeu, quem está em negociação de valores, quem já fechou.
- Perde tempo comparando propostas concorrentes porque não tem visão organizada de todas as etapas (contato inicial, briefing, negociação de cachê, contrato, entrega, pagamento).
- Corre risco de esquecer follow-ups ou perder prazos porque tudo está espalhado em conversas diferentes.

Camila estima gastar cerca de 5 a 6 horas por semana só organizando propostas manualmente — revirando DMs antigas, procurando e-mails perdidos e tentando lembrar em qual etapa cada marca está. Nos últimos três meses, isso já causou dois problemas concretos: ela perdeu uma parceria com uma marca de produtos infantis porque não respondeu a tempo (a proposta ficou "perdida" numa DM do Instagram por 12 dias), e fechou um contrato com uma marca de cosméticos por um valor abaixo do que cobra normalmente, porque não tinha registrado o que outra marca concorrente havia oferecido pelo mesmo tipo de entrega semanas antes.

## O que mudaria
Com um jeito simples e centralizado de mapear cada parceria por etapa, a influencer conseguiria:
- Ver de um lugar só em que estágio está cada proposta (contato → negociação → contrato → entrega → pagamento).
- Fechar parcerias mais rápido, sem perder oportunidades por falta de follow-up.
- Comparar propostas concorrentes com mais clareza antes de negociar valores.

Critério de sucesso: reduzir de 15 dias (média atual, do primeiro contato até a assinatura do contrato) para 7 dias o tempo entre o primeiro contato e a parceria fechada, e eliminar por completo as parcerias perdidas por falta de follow-up (hoje, em média 1 por mês).

## Como eu tenho acesso
Consigo entrevistar semanalmente a Camila, que é minha prima e concordou em participar como usuária-teste ao longo do semestre. Além dela, tenho acesso a mais duas criadoras de conteúdo da minha rede pessoal (uma de moda e outra de fitness, ambas também sem agência, entre 50 mil e 100 mil seguidores), com quem posso conversar quinzenalmente para validar se o problema se repete fora do caso da Camila.

## Por que eu escolhi
Escolhi esse problema porque acompanhei de perto, através da Camila, a frustração de perder uma parceria remunerada por simples desorganização — não por falta de talento ou de audiência, mas por falta de uma ferramenta adequada ao tamanho do trabalho dela. Isso me mostrou uma lacuna real: ferramentas de CRM existentes são feitas para times de vendas, não para criadores individuais que negociam sozinhos. Tenho interesse profissional em construir produtos para o mercado de criadores de conteúdo, que considero um segmento em crescimento e ainda mal atendido por ferramentas simples, e por isso quero usar o semestre para validar e aprofundar esse problema com dados reais.

## O resultado que eu quero
Reduzir o tempo que a Camila gasta organizando propostas de 5-6 horas por semana para cerca de 1 hora por semana — o tempo de revisar o status de cada parceria no board E de responder às propostas, sem precisar revirar DMs e e-mails antigos. Junto com isso, manter o critério já definido: tempo entre primeiro contato e parceria fechada caindo de 15 para 7 dias, e zero parcerias perdidas por falta de follow-up (hoje, em média 1 por mês).

## O plano antes do prompt
- **O que observar:** como as propostas chegam de fato (e-mail, DM do Instagram, DM do TikTok, WhatsApp), em que etapa cada uma trava hoje, e quanto tempo passa entre o contato inicial e a resposta da Camila.
- **O que decidir:** que etapas do funil valem a pena mapear num primeiro momento (contato → negociação → contrato → entrega → pagamento) e qual formato de board é simples o suficiente para a Camila olhar em poucos minutos.
- **O que executo sozinha:** montar e preencher o board de teste com as propostas reais da Camila durante o experimento, a partir do que ela me repassar.
- **O que fica com a Camila:** a negociação de valores e a decisão final de aceitar ou recusar cada proposta — a ferramenta organiza e alerta, quem conversa com a marca e decide o preço continua sendo ela.

## Primeiro experimento
Nas próximas 5 propostas que chegarem para a Camila, vou registrar cada uma manualmente no Notion, com as etapas contato → negociação → contrato → entrega → pagamento, preenchendo eu mesma a partir do que ela me repassar. Critério de sucesso do experimento: zero propostas esquecidas entre as 5 testadas.
